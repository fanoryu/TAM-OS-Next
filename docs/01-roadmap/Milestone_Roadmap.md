...
| **v2.10.0 Official Release** | **Completed · published · Latest** | TAM OS v2.10.0 *Governed Workspace* — **originally published** from annotated tag `v2.10.0` on release commit `335d53ed` in the predecessor `fanoryu/TAM-OS`, then **canonically re-published unchanged** from annotated tag `v2.10.0` in `fanoryu/TAM-OS-Next` (peeling to `856e3ca6`); both repositories show Latest = v2.10.0. Asset `tam-os-v2.10.0.html` (1,151,267 B, SHA-256 `60382271…2c7fa704`) byte-identical across both Releases and to `dist/`. Publication makes the artifact obtainable — it is **not** a new product version, **not** a pilot launch and **not** general availability |
